...
for d in ("sift", "audio"):
  ...
  for m in COMPASSX_METHODS:
    d_m_b_M32_64[d][m] = [f"M_{M}_efc_200_nlist_10000_dx_{dx}" for M in [16, 32] for dx in dataset_args[d]["dx"]]
for d in ("gist", "video", "crawl", "glove100"):
  for m in COMPASS_METHODS:
    d_m_b_M32_64[d][m] = ["M_16_efc_200_nlist_20000", "M_32_efc_200_nlist_20000"]
  for m in COMPASSX_METHODS:
    d_m_b_M32_64[d][m] = [f"M_{M}_efc_200_nlist_20000_dx_{dx}" for M in [16, 32] for dx in dataset_args[d]["dx"]]
