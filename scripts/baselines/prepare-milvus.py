...
if __name__ == "__main__":
  ...
  for d in args.names:
    for da in DA_S:
      ...
      for i in range(da):
        schema.add_field(field_name=f"attr_{i}", datatype=DataType.FLOAT, dim=1)
      ...
      if (vectors.shape[1] != card.dim):
        raise ValueError(f"Loaded vectors have different dimensions for {d}_{da}")

      data = [{
        "id": i, "vector": vector.tolist(), **{
          f"attr_{j}": float(attr[j])
