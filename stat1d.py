...
def stat_top_k_in_cluster_by_selectivity():
  ...
  for nlist in nlist_s:
    for rg in ranges:
      ...
      fig.savefig(f"figures_10/clustering/All-BisectingKMeans-{(rg[1] - rg[0]) / 10000:.1%}-Clustering-{nlist}-Quality.jpg", dpi=100)
      plt.close()
...
def stat_cluster_imbalance_factor():
  ...
  for nlist in nlist_s:
    ...
    fig.set_size_inches(20, 5)
    fig.savefig(f"figures_10/clustering/All-BisectingKMeans-Cluster-{nlist}-Imbalance.jpg", dpi=200)
