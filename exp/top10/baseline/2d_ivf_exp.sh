...
for dataset in ${datasets[@]}; do
  for nlist in ${nlist_s[@]}; do
    ...
    while [ $i -lt ${#r1_s[@]} ]; do
      r1=${r1_s[$i]}
      r2=${r2_s[$i]}
      /home/chunxy/repos/Compass/build/Release/src/benchmarks/bench_compass_ivf --datacard ${dataset}_2_10000_float32 --l 100 200 --r ${r1} ${r2} --k 10 --nlist ${nlist} --nprobe ${nprobe_s[@]}
      i=$((i + 1))
    done
  done
done
