...
def stat_top_k_in_cluster_by_selectivity():
  ...
  stat_tmpl = "kmedoids_top_{}_in_cluster_hist_{}_{}_{}_{}.bin"
  ...
  for nlist in nlist_s:
    for rg in ranges:
      ...
      fig.savefig(f"figures_10/clustering/All-KMedoids-{(rg[1] - rg[0]) / 10000:.1%}-Clustering-{nlist}-Quality.jpg", dpi=100)
      plt.close()


def stat_cluster_imbalance_factor():
  stat_dir = Path(STATS_DIR)
  stat_tmpl = "kmedoids_cluster_element_count_{}_{}.bin"
  nlist_s = [1000, 2000, 5000, 10000, 20000]
  ...
  for nlist in nlist_s:
    ...
    fig.set_size_inches(20, 5)
    fig.savefig(f"figures_10/clustering/All-KMedoids-Cluster-{nlist}-Imbalance.jpg", dpi=200)
