  ...
  for selector in selectors:
    ...
    for m in data.method.unique():
      for b in data[data["method"] == m].build.unique():
        ...
        ax.plot(recall_qps[:, 0], recall_qps[:, 1])
        ax.scatter(recall_qps[:, 0], recall_qps[:, 1], label=f"{m}-{b}", marker=METHOD_MARKER_MAPPING[m])
        ax.set_xlabel('Recall')
        ax.set_ylabel('QPS')
        # plt.xticks(np.arange(0.6, 1.04, 0.05))
        ax.set_title("{}, Selectivity-{:.1%}".format(dataset.capitalize(), selectivity))
...
def draw_1d_comp_recall_by_dataset_selectivity():
...
def draw_1d_by_dataset_eric():
  ...
  for dataset in DATASETS:
    ...
    data = df[df["dataset"] == dataset]
    for m in data.method.unique():
      if m == "CompassRImi1d": continue
      for b in data[data["method"] == m].build.unique():
        rec_qps_sel_comp = data[(data["method"] == m) & (data["build"] == b)][["recall", "qps", "selectivity",
                                                                                "comp"]].sort_values(["selectivity", "recall"])
        cutoff_rec_s = [
          0.8,
        ]
        cutoff_colors = [
        ...
        for cutoff, color in zip(cutoff_rec_s, cutoff_colors):
          grouped_qps = rec_qps_sel_comp[rec_qps_sel_comp["recall"] > cutoff].groupby("selectivity", as_index=False)["qps"].max()
          grouped_comp = rec_qps_sel_comp[rec_qps_sel_comp["recall"] > cutoff].groupby("selectivity", as_index=False)["comp"].min()
          pos_s = np.array([bisect.bisect(selectivities, sel) for sel in grouped_qps["selectivity"]]) - 1
          ax0.plot(
            pos_s,
            grouped_qps["qps"],
          )
          ax0.scatter(pos_s, grouped_qps["qps"], label=f"{m}-{b}-{cutoff}", marker=METHOD_MARKER_MAPPING[m])
          ax1.plot(
            pos_s,
            grouped_comp["comp"],
          )
          ax1.scatter(pos_s, grouped_comp["comp"], label=f"{m}-{b}-{cutoff}", marker=METHOD_MARKER_MAPPING[m])
