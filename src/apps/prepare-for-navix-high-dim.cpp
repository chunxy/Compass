...
int main(int argc, char **argv) {
  ...
  query_file_ss << "queries";
  gt_file_ss << "gt";
  for (int i = 0; i < args.u_bounds.size(); i++) {
    query_file_ss << "_" << (args.u_bounds[i] - args.l_bounds[i]) / 100;
    gt_file_ss << "_" << (args.u_bounds[i] - args.l_bounds[i]) / 100;
  }
  ...
  for (int i = 0; i < 200; i++) {
    ...
    predicate << "e.attr0 >= " << args.l_bounds[0] << " AND e.attr0 <= " << args.u_bounds[0];
    for (int j = 1; j < args.l_bounds.size(); j++) {
      predicate << fmt::format(" AND e.attr{} >= {} AND e.attr{} <= {}", j, args.l_bounds[j], j, args.u_bounds[j]);
    }
