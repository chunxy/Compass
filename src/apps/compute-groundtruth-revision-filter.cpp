...
int main(int argc, char **argv) {
  ...
  values.resize(c.n_queries * c.attr_dim);
  ...
  memcpy(values.data(), rg, c.n_queries * c.attr_dim * sizeof(float));

  if (values.size() != c.n_queries * c.attr_dim) {
    fmt::print("Attribute dimension mismatch: {} != {}\n", values.size(), c.n_queries * c.attr_dim);
    return 1;
  }
  ...
  compute_groundtruth(xb, nb, xq, nq, d, c.attr_dim, values.data(), attrs, k, c.type, nsat, hybrid_topks);
