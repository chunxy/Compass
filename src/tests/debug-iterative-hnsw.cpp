...
int main(int argc, char **argv) {
  ...
  string card_name = "siftsmall_1_1000_float32";
  DataCard c = name_to_card[card_name];
  ...
  int M = 16, efc = 200;
  int k = nb;  // Forcibly find all the base vectors.
  int batch_k = 100, initial_efs = 100;
  vector<int> delta_efs_s = {100};
  ...
  // Load groundtruth for similarity search.
  ...
  while (!groundtruth_it.HasEnded()) {
    ...
    hybrid_topks[i].resize(ng);
    memcpy(hybrid_topks[i].data(), next.data(), ng * sizeof(uint32_t));
    i++;
  }
