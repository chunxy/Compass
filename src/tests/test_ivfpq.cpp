...
int main() {
  ...
  {  // search xq
    ...
    // printf("I=\n");
    // for (int i = 0; i < nq; i++) {
    //   for (int j = 0; j < k; j++) printf("%7zd ", I[i * k + j]);
    //   printf("\n");
    // }
    ...
    for (int i = 0; i < nq; i++) {
      ...
      auto largest_qt_dist = D[i * k + k - 1];
      auto largest_dist = func(xb + d * I[i * k + k - 1], xq + d * i, &d);
      printf(
          "largest GT dist: %7g, largest QT dist: %7g, largest dist: %7g\n",
          largest_gt_dist,
          largest_qt_dist,
          largest_dist
      );
