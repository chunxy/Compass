...
int main() {
  ...
  faiss::IndexIVFScalarQuantizer index(
      new faiss::IndexScalarQuantizer(d, faiss::ScalarQuantizer::QuantizerType::QT_8bit, faiss::METRIC_L2),
      d,
      100,
  ...
  {  // search xq
    ...
    auto encode_beg = std::chrono::high_resolution_clock::now();
    uint8_t *quantized_xq = new uint8_t[nq * index.code_size];
    index.quantizer->sa_encode(nq, xq, quantized_xq);
    auto encode_end = std::chrono::high_resolution_clock::now();
    auto encode_duration = std::chrono::duration_cast<std::chrono::microseconds>(encode_end - encode_beg);
    ...
    search_duration -= encode_duration;
