...
int main(int argc, char **argv) {
  ...
  fs::path log_root("/home/chunxy/repos/Compass/scratches/test-iterative-hnsw-batch");
  ...
  for (auto efs : delta_efs_s) {
    ...
    for (int j = 0; j < nq; j++) {
      ...
      vl->reset();
      IterativeSearchState<float> state = comp->Open(xq + j * d, k, vl);
      ...
      while (rz_indices.size() < k) {
        ...
        auto batch = comp->NextBatch(&state);
