...
#include "methods/acorn/IndexHNSW.h"
...
#include "methods/acorn/auxiliary.h"
...
void hnsw_add_vertices(
  ...
  idx_t check_period =
      InterruptCallback::get_period_hint(max_level * index_hnsw.d * hnsw.efConstruction);
...
void IndexHNSW::train(idx_t n, const float *x) {
  FAISS_THROW_IF_NOT_MSG(
      storage, "Please use IndexHNSWFlat (or variants) instead of IndexHNSW directly"
  );
...
void IndexHNSW::partition_search(
  ...
  idx_t check_period = InterruptCallback::get_period_hint(
      partition_indices[num_partitions - 1]->hnsw.max_level * d * efSearch
  );
...
void IndexHNSW::link_singletons() {
  ...
  printf(
      "  Found %d / %" PRId64 " singletons (%d appear in a level above)\n",
      n_sing,
      ntotal,
      n_sing_l1
  );
...
void ReconstructFromNeighbors::reconstruct_n(storage_idx_t n0, storage_idx_t ni, float *x)
    const {
#pragma omp parallel
  {
...
void IndexHNSW2Level::search(
  ...
  if (dynamic_cast<const Index2Layer *>(storage)) {
    ...
    index_ivfpq->search_preassigned(
        n, x, k, coarse_assign.get(), coarse_dis.get(), distances, labels, false
    );
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
      for (idx_t i = 0; i < n; i++) {
        ...
        search_from_candidates_2(
            hnsw, *dis, k, idxi, simi, candidates, vt, search_stats, 0, k
        );
        n1 += search_stats.n1;
        n2 += search_stats.n2;
        n3 += search_stats.n3;
...
void IndexHNSW2Level::flip_to_ivf() {
  ...
  IndexIVFPQ *index_ivfpq =
      new IndexIVFPQ(storage2l->q1.quantizer, d, storage2l->q1.nlist, storage2l->pq.M, 8);
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
      for (idx_t i = i0; i < i1; i++) {
        ...
        HNSWStats stats = hnsw.hybrid_search(
            *dis, k, idxi, simi, vt, filters[i], op, regex, params
        );  // TODO edit to hybrid search
        n1 += stats.n1;
        n2 += stats.n2;
        n3 += stats.n3;
        ndis += stats.ndis;
