  ...
  if (filtered_edge_list) {
    ...
    for (int level = 0; level <= max_level; level++) {
      ...
      print_edges_filtered(
          level, filter, op
      );  // this is more detailed version with edge lists
    }
  }
  // print metadata
  // printf("========= METADATA =======\n");
...
void ACORN::shrink_neighbor_list(
  ...
  debug(
      "shrink_neighbor_list: input size: %ld, max_size: %d, gamma: %d\n",
      input.size(),
      max_size,
      gamma
  );
...
void shrink_neighbor_list(
  ...
  debug(
      "shrink_neighbor_list from size %ld, to max size %d\n", resultSet1.size(), max_size
  );
...
void add_link(
  ...
  if (level == 0) {
    shrink_neighbor_list(
        qdis, resultSet, end - begin, hnsw.gamma, src, hnsw.metadata[src], hnsw
    );
  }
...
void ACORN::add_links_starting_from(
  ...
  if (level == 0) {
    ::acorn::shrink_neighbor_list(
        ptdis, link_targets, M, gamma, pt_id, this->metadata[pt_id], *this
    );
    // printf("shrunk");
  }
