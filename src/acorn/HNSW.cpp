...
int HNSW::cum_nb_neighbors(int layer_no) const { return cum_nneighbor_per_level[layer_no]; }

void HNSW::neighbor_range(idx_t no, int layer_no, size_t *begin, size_t *end) const {
  size_t o = offsets[no];
  // debug("offset o: %ld\n", o);
...
#pragma omp parallel for reduction(+ : tot_neigh) reduction(+ : tot_common) reduction(+ : tot_reciprocal) \
    reduction(+ : n_node)
  for (int i = 0; i < levels.size(); i++) {
    if (levels[i] > level) {
      n_node++;
  ...
  printf(
      "   4. nb of neighbors that are also neighbor-of-neighbors: %.2f (%zd)\n", tot_common / normalizer, tot_common
  );
}
...
void HNSW::print_edges(int level) const {
  ...
  printf(
      "   5. nb of neighbors that are also neighbor-of-neighbors: %.2f (%zd)\n", tot_common / normalizer, tot_common
  );
}
...
void add_link(HNSW &hnsw, DistanceComputer &qdis, storage_idx_t src, storage_idx_t dest, int level) {
  size_t begin, end;
...
/// add a link between two elements, possibly shrinking the list
/// of links to make room for it.
void hybrid_add_link(HNSW &hnsw, DistanceComputer &qdis, storage_idx_t src, storage_idx_t dest, int level) {
  size_t begin, end;
  hnsw.neighbor_range(src, level, &begin, &end);
  if (hnsw.hybrid_neighbors[end - 1].first == -1) {  // access to std::pair
    ...
    size_t i = end;
    while (i > begin) {
      if (hnsw.hybrid_neighbors[i - 1].first != -1) break;
      i--;
    }
...
void search_neighbors_to_add(
  ...
  while (!candidates.empty()) {
    ...
    // debug("during BFS, gamma: %d, candidates size: %ld, results size: %ld, vt.num_visited: %d, nb on level: %d,
    // backtrack_level: %d, level: %d\n", hnsw.gamma, candidates.size(), results.size(), vt.num_visited(),
    // hnsw.nb_per_level[level], backtrack_level, level); if (hnsw.gamma > 1 && candidates.empty() && results.size() < M
    // && vt.num_visited() < hnsw.nb_per_level[level] && backtrack_level <= level) { if (hnsw.gamma > 1 &&
    // candidates.empty() && results.size() < M && vt.num_visited() < hnsw.nb_per_level[level]) {
    ...
    //         hnsw.neighbor_range(ep_per_level[backtrack_level], backtrack_level, &begin, &end); // TODO can also do
    //         this for query instead of ep debug("\tchecking %ld neighbors of node %d at level %d\n", (end - begin),
    //         entry_point, backtrack_level); for (size_t i = begin; i < end; i++) {
    //             // debug("\t\titer %d\n", i);
    //             storage_idx_t nodeId = hnsw.neighbors[i];
...
void hybrid_search_neighbors_to_add(
  ...
  while (!candidates.empty()) {
    ...
    // debug("during BFS, gamma: %d, candidates size: %ld, results size: %ld, vt.num_visited: %d, nb on level: %d,
    // backtrack_level: %d, level: %d\n", hnsw.gamma, candidates.size(), results.size(), vt.num_visited(),
    // hnsw.nb_per_level[level], backtrack_level, level);
  }
  // debug("search_neighbors to add finds %ld nn's\n", results.size());
  vt.advance();
}
...
int hybrid_greedy_update_nearest(
  ...
  for (;;) {
    ...
    for (size_t i = begin; i < end; i++) {
      ...
      // }
      // check filter
      if (((op == EQUAL) && (hnsw.metadata[v] == filter)) || (op == OR && (hnsw.metadata[v] & filter != 0)) ||
      ...
      if (((op == EQUAL) && (hnsw.metadata[v] == filter)) || (op == OR && (hnsw.metadata[v] & filter != 0)) ||
        ...
        float dis = qdis(v);
        ndis++;
        if (dis < d_nearest || ((op == EQUAL) && (hnsw.metadata[nearest] != filter)) ||
            (op == OR && (hnsw.metadata[nearest] & filter == 0)) ||
            (op == REGEX && !CHECK_REGEX(hnsw.metadata_strings[nearest], regex))) {
          nearest = v;
          d_nearest = dis;
        }
        if (num_found >= hnsw.M) {
      ...
      for (size_t j = begin2; j < end2; j++) {
        ...
        // check filter
        if (((op == EQUAL) && (hnsw.metadata[v2] == filter)) || (op == OR && (hnsw.metadata[v2] & filter != 0)) ||
            (op == REGEX && CHECK_REGEX(hnsw.metadata_strings[v2], regex))) {
          num_found = num_found + 1;
          float dis2 = qdis(v2);
          ndis += 1;

          if (dis2 < d_nearest || ((op == EQUAL) && (hnsw.metadata[nearest] != filter)) ||
              (op == OR && (hnsw.metadata[nearest] & filter == 0)) ||
              (op == REGEX && !CHECK_REGEX(hnsw.metadata_strings[nearest], regex))) {
            nearest = v2;
            d_nearest = dis2;
...
void HNSW::add_links_starting_from(
  ...
  search_neighbors_to_add(*this, ptdis, link_targets, nearest, d_nearest, level, vt, ep_per_level);
  ...
  ::acorn::shrink_neighbor_list(ptdis, link_targets, M, gamma);
  ...
  while (!link_targets.empty()) {
    ...
    );  // for hybrid search we also will store the metadata in the neighbor list
...
void HNSW::hybrid_add_links_starting_from(
  ...
  hybrid_search_neighbors_to_add(*this, ptdis, link_targets, nearest, d_nearest, level, vt, ep_per_level);
...
#pragma omp critical
  ...
  for (; level >= 0; level--) {
    ...
    add_links_starting_from(ptdis, pt_id, nearest, d_nearest, level, locks.data(), vt, ep_per_level);
...
int hybrid_search_from_candidates(
  ...
  while (candidates.size() > 0) {  // candidates is heap of size max(efs, k)
    ...
    for (size_t j = begin; j < end; j++) {
      ...
      if (v1 < 0) break;
      // check filter
      if (((op == EQUAL) && (hnsw.metadata[v1] == filter)) || (op == OR && ((hnsw.metadata[v1] & filter) != 0)) ||
          (op == REGEX && CHECK_REGEX(hnsw.metadata_strings[v1], regex))) {
        num_found = num_found + 1;
      }
      if (vt.get(v1)) {
        continue;
...
HNSWStats
HNSW::search(DistanceComputer &qdis, int k, idx_t *I, float *D, VisitedTable &vt, const SearchParametersHNSW *params)
    const {
  // debug("%s\n", "reached");
  HNSWStats stats;
  if (entry_point == -1) {
  ...
  if (upper_beam == 1) {  // common branch
    ...
    if (search_bounded_queue) {  // this is the most common branch
      ...
      std::priority_queue<Node> top_candidates =
          search_from_candidate_unbounded(*this, Node(d_nearest, nearest), qdis, ef, &vt, stats);
    ...
    for (int level = max_level; level >= 0; level--) {
      ...
      if (level == 0) {
        nres = search_from_candidates(*this, qdis, k, I, D, candidates, vt, stats, 0);
      } else {
        nres = search_from_candidates(
            *this, qdis, candidates_size, I_to_next.data(), D_to_next.data(), candidates, vt, stats, level
        );
      }
      vt.advance();
    }
...
HNSWStats HNSW::hybrid_search(
  ...
  if (upper_beam == 1) {  // common branch
    ...
    if (search_bounded_queue) {  // this is the most common branch
      ...
      hybrid_search_from_candidates(*this, qdis, filter, op, regex, k, I, D, candidates, vt, stats, 0, 0, params);
    } else {
      // TODO
    ...
    for (int level = max_level; level >= 0; level--) {
      ...
      if (level == 0) {
        nres = hybrid_search_from_candidates(*this, qdis, filter, op, regex, k, I, D, candidates, vt, stats, 0);
      } else {
...
void HNSW::search_level_0(
  ...
  if (search_type == 1) {
    ...
    for (int j = 0; j < nprobe; j++) {
      ...
      nres = search_from_candidates(hnsw, qdis, k, idxi, simi, candidates, vt, search_stats, 0, nres);
    }
    ...
    search_from_candidates(hnsw, qdis, k, idxi, simi, candidates, vt, search_stats, 0);
  }
}
