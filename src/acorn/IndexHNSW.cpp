...
using faiss::fvec_argsort;
using faiss::fvec_L2sqr;
using faiss::getmillisecs;
using faiss::Index2Layer;
using faiss::IndexFlat;
using faiss::IndexIVFPQ;
using faiss::IndexPQ;
using faiss::IndexScalarQuantizer;
using faiss::InterruptCallback;
using faiss::maxheap_heapify;
using faiss::maxheap_reorder;
using faiss::METRIC_INNER_PRODUCT;
using faiss::RandomGenerator;
using faiss::ScopeDeleter;
using faiss::ScopeDeleter1;
...
struct NegativeDistanceComputer : DistanceComputer {
  ...
  explicit NegativeDistanceComputer(DistanceComputer *basedis) : basedis(basedis) {}
  ...
  float symmetric_dis(idx_t i, idx_t j) override { return -basedis->symmetric_dis(i, j); }
...
void hnsw_add_vertices(
  ...
  idx_t check_period = InterruptCallback::get_period_hint(max_level * index_hnsw.d * hnsw.efConstruction);
...
#pragma omp parallel if (i1 > i0 + 100)
      {
        ...
        DistanceComputer *dis = storage_distance_computer(index_hnsw.storage);
...
IndexHNSW::IndexHNSW(int d, int M, int gamma, MetricType metric)
    : Index(d, metric), hnsw(M, gamma), own_fields(false), storage(nullptr), reconstruct_from_neighbors(nullptr) {}
...
void IndexHNSW::train(idx_t n, const float *x) {
  FAISS_THROW_IF_NOT_MSG(storage, "Please use IndexHNSWFlat (or variants) instead of IndexHNSW directly");
...
void IndexHNSW::search(
  ...
  idx_t check_period = InterruptCallback::get_period_hint(hnsw.max_level * d * efSearch);
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
      for (idx_t i = i0; i < i1; i++) {
        ...
        if (reconstruct_from_neighbors && reconstruct_from_neighbors->k_reorder != 0) {
          int k_reorder = reconstruct_from_neighbors->k_reorder;
          if (k_reorder == -1 || k_reorder > k) k_reorder = k;

          nreorder += reconstruct_from_neighbors->compute_distances(k_reorder, idxi, x + i * d, simi);
...
void IndexHNSW::partition_search(
    ...
    const float *x,
    idx_t k,
    std::vector<acorn::IndexHNSW *> &partition_indices,
    int num_partitions,
    float *distances,
    idx_t *labels,
    int *filters,
...
void IndexHNSW::reconstruct(idx_t key, float *recons) const { storage->reconstruct(key, recons); }
...
#pragma omp for
    for (idx_t i = 0; i < ntotal; i++) {
      ...
      HNSW::shrink_neighbor_list(*dis, initial_list, shrunk_list, new_size);
...
void IndexHNSW::init_level_0_from_knngraph(int k, const float *D, const idx_t *I) {
...
void IndexHNSW::init_level_0_from_entry_points(int n, const storage_idx_t *points, const storage_idx_t *nearests) {
  std::vector<omp_lock_t> locks(ntotal);
  for (int i = 0; i < ntotal; i++) omp_init_lock(&locks[i]);
...
#pragma omp for schedule(dynamic)
    for (int i = 0; i < n; i++) {
      ...
      hnsw.add_links_starting_from(*dis, pt_id, nearest, (*dis)(nearest), 0, locks.data(), vt);
...
ReconstructFromNeighbors::ReconstructFromNeighbors(const IndexHNSW &index, size_t k, size_t nsq)
...
void ReconstructFromNeighbors::reconstruct(storage_idx_t i, float *x, float *tmp) const {
  ...
  if (k == 1 || nsq == 1) {
    ...
    for (int l = 0; l < d; l++) x[l] = w0 * tmp[l];
    ...
    w0 = beta0[0];
    for (int l = 0; l < dsub; l++) x[l] = w0 * tmp[l];

    w0 = beta1[0];
    for (int l = dsub; l < d; l++) x[l] = w0 * tmp[l];
    ...
    for (size_t j = begin; j < end; j++) {
      ...
      float w;
      w = beta0[j - begin + 1];
      for (int l = 0; l < dsub; l++) x[l] += w * tmp[l];

      w = beta1[j - begin + 1];
      for (int l = dsub; l < d; l++) x[l] += w * tmp[l];
    }
  } else {
...
void ReconstructFromNeighbors::reconstruct_n(storage_idx_t n0, storage_idx_t ni, float *x) const {
#pragma omp parallel
  {
...
/// called by add_codes
void ReconstructFromNeighbors::estimate_code(const float *x, storage_idx_t i, uint8_t *code) const {
  ...
  for (size_t sq = 0; sq < nsq; sq++) {
...

    {
      ...
      sgemm_(
          "N", "N", &dsubi, &ki, &m1, &one, tmp1 + d0, &di, codebook.data() + sq * (m1 * k), &m1, &zero, tmp2, &dsubi
      );
    }
...
#pragma omp parallel for
  for (int i = 0; i < n; i++) {
    estimate_code(x + i * index.d, ntotal + i, codes.data() + (ntotal + i) * code_size);
  }
  ntotal += n;
...
IndexHNSWPQ::IndexHNSWPQ(int d, int pq_m, int M) : IndexHNSW(new IndexPQ(d, pq_m, 8), M) {
  own_fields = true;
  is_trained = false;
}
...
IndexHNSWSQ::IndexHNSWSQ(int d, ScalarQuantizer::QuantizerType qtype, int M, MetricType metric)
...
void IndexHNSW2Level::search(
  ...
  if (dynamic_cast<const Index2Layer *>(storage)) {
    ...
    const IndexIVFPQ *index_ivfpq = dynamic_cast<const IndexIVFPQ *>(storage);
    ...
    index_ivfpq->quantizer->search(n, x, nprobe, coarse_dis.get(), coarse_assign.get());

    index_ivfpq->search_preassigned(n, x, k, coarse_assign.get(), coarse_dis.get(), distances, labels, false);
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
      for (idx_t i = 0; i < n; i++) {
        ...
        HNSWStats search_stats;
        search_from_candidates_2(hnsw, *dis, k, idxi, simi, candidates, vt, search_stats, 0, k);
        n1 += search_stats.n1;
        n2 += search_stats.n2;
        n3 += search_stats.n3;
...
void IndexHNSW2Level::flip_to_ivf() {
  ...
  IndexIVFPQ *index_ivfpq = new IndexIVFPQ(storage2l->q1.quantizer, d, storage2l->q1.nlist, storage2l->pq.M, 8);
...
IndexHNSWHybridOld::IndexHNSWHybridOld() { is_trained = true; }
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
      for (idx_t i = i0; i < i1; i++) {
        ...
        HNSWStats stats =
            hnsw.hybrid_search(*dis, k, idxi, simi, vt, filters[i], op, regex, params);  // TODO edit to hybrid search
        n1 += stats.n1;
        n2 += stats.n2;
        n3 += stats.n3;
        ndis += stats.ndis;
