...
void ACORN::print_neighbor_stats(bool edge_list, bool filtered_edge_list, int filter, Operation op) const {
  ...
  if (filtered_edge_list) {
    printf(
        "========= LEVEL STATS OF SUBGRAPH WITH FILTER %d, OP %d (0 for EQUAL, 1 for "
        "OR)=======\n",
        filter,
        op
    );
...
void shrink_neighbor_list(
  ...
  // ACORN::shrink_neighbor_list(qdis, resultSet, returnlist, max_size, gamma, q_id,
  // q_attr);
...
void add_link(ACORN &hnsw, DistanceComputer &qdis, storage_idx_t src, storage_idx_t dest, int level) {
  size_t begin, end;
...
void search_neighbors_to_add(
  ...
  while (!candidates.empty()) {
    ...
    if ((currEv.d > results.top().d && hnsw.gamma == 1) || results.size() >= M) {
      debug(
          "greedy stop in construction, results size: %ld, desired size: %d, gamma = "
          "%d\n",
          results.size(),
          M,
          hnsw.gamma
    ...
    for (size_t i = begin; i < end; i++) {
      ...
      // debug("while checking neighbors of %d, efc: %d, results size: %ld, just visited
      ...
      debug(
          "while checking neighbors of %d, just visited %d -- efc: %d, results size: "
          "%ld, candidates size: %ld, \n",
          currNode,
          nodeId,
          hnsw.efConstruction,
          results.size(),
    ...
    debug(
        "during BFS, gamma: %d, candidates size: %ld, results size: %ld, vt.num_visited: "
        "%d, nb on level: %d, backtrack_level: %d, level: %d\n",
        hnsw.gamma,
        candidates.size(),
        results.size(),
        vt.num_visited(),
...
int hybrid_greedy_update_nearest(
  ...
  debug("%s\n", "reached");
  // printf("hybrid_greedy_update_nearest called with parameters: filter: %d, op: %d,
  ...
  for (;;) {
    ...
    for (size_t i = begin; i < end; i++) {
      ...
      //     debug_search("------------checking neighbor: %d, metadata: %d, metadata &
      //     filter: %d\n", v, metadata, metadata & filter);
      // }

      // filter
      // printf("---at first filter: op: %d, metadata: %s, regex: %s, check_regex result:
      // %d\n", op, hnsw.metadata_strings[v].c_str(), regex.c_str(),
...
void ACORN::add_links_starting_from(
  ...
  search_neighbors_to_add(*this, ptdis, link_targets, nearest, d_nearest, level, vt, ep_per_level);  // mod
  ...
  if (level == 0) {
    ::acorn::shrink_neighbor_list(ptdis, link_targets, M, gamma, pt_id, this->metadata[pt_id], *this);
    // printf("shrunk");
  }
...
#pragma omp critical
  ...
  // needed for backtracking in hybrid search - TODO DEPRECATED, remove backtracking
  // things
...
int hybrid_search_from_candidates(
  ...
  // debug("%s\n", "reached");
  // printf("----hybrid_search_from_candidates called with filter: %d, k: %d, op: %d,
  ...
  while (candidates.size() > 0) {  // candidates is heap of size max(efs, k)
    ...
    for (size_t j = begin; j < end; j++) {
      ...
      if (filter_map[v1]) {
        ...
        if (!sel || sel->is_member(v1)) {
          if (nres < k) {
            // debug_search("-----------------pushing new candidate, nres: %d (to be
            // incrd)\n", nres);
            faiss::maxheap_push(++nres, D, I, d, v1);
        ...
        if (num_found >= hnsw.M * 2) {
          // debug_search("------------num_found: %d, M: %d - triggered outer brea,
          // skpping to M_beta=%d neighbork\n", num_found, hnsw.M * 2, hnsw.M_beta);
      ...
      if (((j - begin >= hnsw.M_beta) && keep_expanding) || hnsw.gamma == 1) {
        ...
        for (size_t j2 = begin2; j2 < end2; j2 += 1) {
          ...
          // if (debugSearchFlag) {
          //     neighbors_checked.push_back(std::make_pair(v2, metadata2)); // for
          //     debugging
          // }
          if (v2 < 0) {
            // continue;
...
ACORNStats
ACORN::search(DistanceComputer &qdis, int k, idx_t *I, float *D, VisitedTable &vt, const SearchParametersACORN *params)
    const {
  debug("%s\n", "reached");
  ACORNStats stats;
  if (entry_point == -1) {
...
ACORNStats ACORN::hybrid_search(
  ...
  if (upper_beam == 1) {  // common branch
    ...
    for (int level = max_level; level >= 1; level--) {
      debug_search(
          "-at level %d, searching for greedy nearest from current nearest: %d, dist: "
          "%f, metadata: %d\n",
          level,
          nearest,
          d_nearest,
      ...
      // ndis_upper += hybrid_greedy_update_nearest(*this, qdis, filter, op, regex, level,
      // nearest, d_nearest);
      debug_search(
    ...
    if (search_bounded_queue) {  // this is the most common branch
      ...
      hybrid_search_from_candidates(*this, qdis, filter_map, k, I, D, candidates, vt, stats, 0, 0, params);
    ...
    for (int level = max_level; level >= 0; level--) {
      ...
      if (level == 0) {
        nres = hybrid_search_from_candidates(*this, qdis, filter_map, k, I, D, candidates, vt, stats, 0);
...
float ACORN::MinimaxHeap::max() const { return dis[0]; }

int ACORN::MinimaxHeap::size() const { return nvalid; }

void ACORN::MinimaxHeap::clear() { nvalid = k = 0; }
