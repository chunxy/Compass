...
void acorn_add_vertices(
  ...
  if (verbose) {
    printf(
        "acorn_add_vertices: adding %zd elements on top of %zd "
        "(preset_levels=%d)\n",
        n,
        n0,
        int(preset_levels)
  ...
  idx_t check_period = InterruptCallback::get_period_hint(max_level * index_acorn.d * acorn.efConstruction);
...
void IndexACORN::train(idx_t n, const float *x) {
  FAISS_THROW_IF_NOT_MSG(storage, "Please use IndexACORNFlat (or variants) instead of IndexACORN directly");
...
void IndexACORN::search(
  ...
  idx_t check_period = InterruptCallback::get_period_hint(acorn.max_level * d * efSearch);
...
#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder, candidates_loop)
      for (idx_t i = i0; i < i1; i++) {
        ...
        // ACORNStats stats = acorn.hybrid_search(*dis, k, idxi, simi, vt, filters[i], op, regex, params); //TODO edit
        // to hybrid search
        n1 += stats.n1;
        n2 += stats.n2;
        n3 += stats.n3;
  ...
  acorn_stats.combine({n1, n2, n3, ndis, nreorder, candidates_loop, neighbors_loop, tuple_unwrap, skips, visits}
  );  // added for profiling
}
...
void IndexACORN::reconstruct(idx_t key, float *recons) const { storage->reconstruct(key, recons); }
