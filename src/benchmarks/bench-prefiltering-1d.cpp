...
int main(int argc, char **argv) {
  ...
  btree::btree_map<float, labeltype> btree;
  for (int i = 0; i < nb; i++) {
    btree.insert({attrs[i], labels[i]});
  }
  L2Space space(d);
...
// #pragma omp parallel for num_threads(args.nthread) schedule(static)
  ...
  for (int j = 0; j < nq; j++) {
    ...
    auto beg = btree.lower_bound(args.l_bound);
    auto end = btree.upper_bound(args.u_bound);
    num_computations[j] = std::distance(beg, end);
    while (beg != end) {
      int i = beg->second;
      beg++;
#ifdef USE_SSE
