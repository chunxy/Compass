...
int main(int argc, char **argv) {
  ...
  CompassIvf<float, float> comp(d, nb, args.nlist, args.nprobe, xb);
...
#pragma omp parallel for
  ...
  for (int j = 0; j < nq; j += args.batchsz) {
    ...
    auto rz =
        comp.SearchKnn(xq + j * d, args.batchsz, args.k, args.l_bounds, args.u_bounds, args.nprobe, metrics);
  }
