...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassKQicg";
  ...
  if (fs::exists(ckp_dir / cgraph_ckp) && fs::exists(ckp_dir / sq_ckp)) {
    comp.LoadClusterGraph((ckp_dir / cgraph_ckp).string());
    comp.LoadScalarQuantizer((ckp_dir / sq_ckp).string());
    ...
    comp.SaveScalarQuantizer(ckp_dir / sq_ckp);
  }
