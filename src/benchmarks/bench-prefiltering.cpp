...
int main(int argc, char **argv) {
  ...
  fc::BTreeMultiMap<float, pair<labeltype, array<float, 4>>> btree;
  for (int i = 0; i < nb; i++) {
    array<float, 4> arr = {0, 0, 0, 0};
    for (int j = 0; j < c.attr_dim; j++) {
      arr[j] = attrs[i * c.attr_dim + j];
    }
    btree.insert(
        fc::BTreePair<float, pair<labeltype, array<float, 4>>>(std::move(attrs[i * c.attr_dim + 0]), {labels[i], arr})
    );
  }
  L2Space space(d);
...
// #pragma omp parallel for num_threads(args.nthread) schedule(static)
  ...
  for (int j = 0; j < nq; j++) {
    ...
    auto beg = btree.lower_bound(args.l_bounds[0]);
    auto end = btree.upper_bound(args.u_bounds[0]);
    ...
    while (beg != end) {
      ...
      for (int dim = 1; dim < c.attr_dim; dim++) {
        if (attr[dim] < args.l_bounds[dim] || attr[dim] > args.u_bounds[dim]) {
          ok = false;
          break;
        }
      }
