...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassPostKThBs";
  ...
  for (auto efs : args.efs) {
    for (auto nrel : args.nrel) {
      ...
      BitsetQuery<float> bitset(args.l_bounds.data(), args.u_bounds.data(), attrs, nb, c.attr_dim);
...
// #pragma omp taskloop
      ...
      for (int j = 0; j < nq; j += args.batchsz) {
        ...
        results[b] = comp.SearchKnnPostFilteredTwoHopGivenBitset(
            xq + j * d,
            args.batchsz,
            args.k,
            attrs,
