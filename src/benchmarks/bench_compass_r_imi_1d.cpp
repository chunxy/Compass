...
#include "config.h"
#include "json.hpp"
#include "methods/CompassRImi1d.h"
...
int main(int argc, char **argv) {
  ...
  int nsub = 4;
  int nbits = 4;  // the number of bits to represent the sub-centroid
  ...
  std::string method = "CompassRImi1d";
  std::string workload =
      fmt::format(HYBRID_WORKLOAD_TMPL, c.name, c.attr_range, args.l_bound, args.u_bound, args.k);
  std::string build_param = fmt::format("M_{}_efc_{}_nsub_{}_nbits_{}", args.M, args.efc, nsub, nbits);
  ...
  CompassRImi1d<float, float> comp(d, args.M, args.efc, nb, nsub, nbits, args.nrel);
  ...
  std::string ivf_ckp = fmt::format(COMPASS_IMI_CHECKPOINT_TMPL, nsub, nbits);
...
// #pragma omp taskloop
  ...
  for (int j = 0; j < nq;) {
    ...
    for (int ii = 0; ii < results.size(); ii++) {
      ...
      stat.ivf_ppsl_qlty[j] = (double)ivf_ppsl_in_tp / stat.ivf_ppsl_nums[j];
      stat.ivf_ppsl_rate[j] = (double)ivf_ppsl_in_rz / stat.ivf_ppsl_nums[j];
      stat.graph_ppsl_qlty[j] = (double)graph_ppsl_in_tp / stat.graph_ppsl_nums[j];
      stat.graph_ppsl_rate[j] = (double)graph_ppsl_in_rz / stat.graph_ppsl_nums[j];
      stat.perc_of_ivf_ppsl_in_tp[j] = (double)ivf_ppsl_in_tp / rz_gt_interse.size();
