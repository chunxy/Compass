...
int main(int argc, char **argv) {
  ...
  fc::BTreeMultiMap<float, pair<labeltype, array<float, 2>>> btree;
  for (int i = 0; i < nb; i++) {
    btree.insert(fc::BTreePair<float, pair<labeltype, array<float, 2>>>(
        std::move(attrs[i * c.attr_dim + 0]), {labels[i], {attrs[i * c.attr_dim + 0], attrs[i * c.attr_dim + 1]}}
    ));
  }
  L2Space space(d);
