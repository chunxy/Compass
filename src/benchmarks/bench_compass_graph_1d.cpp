...
int main(int argc, char **argv) {
  ...
  CompassGraph1d<float, float> comp(d, args.M, args.efc, nb);
...
// #pragma omp parallel for num_threads(args.nthread) schedule(static)
      ...
      for (int j = 0; j < nq; j += args.batchsz) {
        comp.SearchKnn(xq + j * d, args.batchsz, args.k, args.l_bound, args.u_bound, efs, nrel, metrics);
      }
      ...
      for (int j = 0; j < nq;) {
        vector<Metric> metrics(args.batchsz, Metric(nb));
        auto search_start = high_resolution_clock::now();
        auto results = comp.SearchKnn(xq + j * d, args.batchsz, args.k, args.l_bound, args.u_bound, efs, nrel, metrics);
        auto search_stop = high_resolution_clock::now();
