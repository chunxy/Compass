...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassGraph";
  std::string workload =
  ...
  CompassGraph<float, float> comp(d, args.M, args.efc, nb, args.nrel);
  ...
  comp.AddAttrs(nb, attrs, labels.data());
...
// #pragma omp parallel for num_threads(args.nthread) schedule(static)
  ...
  for (int j = 0; j < nq; j += args.batchsz) {
    ...
    auto rz =
        comp.SearchKnn(xq + j * d, args.batchsz, args.k, args.l_bounds, args.u_bounds, args.efs, metrics);
