  ...
  for (auto efs : efs_s) {
    ...
    for (int b = 0; b < n_batches; b++) {
      for (int i = 0; i < batch_size; i++) {
        int id = (b * batch_size + i) * c.attr_dim;
        if (c.type == "onesided") {
          filter = new GeqQuery<float>(l_bounds[id], attrs, c.attr_dim);
        } else if (c.type == "point") {
          filter = new PointQuery<float>(l_bounds[id], attrs, c.attr_dim);
        } else if (c.type == "negation") {
          filter = new NegationQuery<float>(l_bounds[id], attrs, c.attr_dim);
        }

        for (int j = 0; j < nb; j++) {
          filter_id_map[i * nb + j] = (*filter)(j);
        }
      }
      hybrid_index->search(
          batch_size,
