...
#include "json.hpp"
#include "methods/CompassRProclus1d.h"
...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassRR1dProclus";
  ...
  if (fs::exists(ckp_root / "Proclus" / c.name / subspace_ckp)) {
    ...
    fmt::print("Cannot find subspace mask. Exitting now.\n");
    return -1;
  }
  ...
  if (fs::exists(ckp_root / "Proclus" / c.name / rank_ckp)) {
    ...
    // in.read((char *)proclus.assignment, nb * sizeof(int32_t));
    comp.LoadRanking(ckp_root / "Proclus" / c.name / rank_ckp, attrs.data());
    fmt::print("Finished loading ranking.\n");
  } else {
    ...
    comp.SaveRanking(ckp_root / "Proclus" / c.name / rank_ckp);
  }
