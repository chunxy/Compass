...
int main(int argc, char **argv) {
  ...
  Compass1dKCg<float, float> comp(nb, d, args.M, args.efc, args.nlist, args.M_cg);
  ...
  if (fs::exists(ckp_root / "KMedoids" / c.name / cgraph_ckp)) {
    comp.LoadClusterGraph((ckp_root / "KMedoids" / c.name / cgraph_ckp).string());
    ...
    comp.SaveClusterGraph(ckp_root / "KMedoids" / c.name / cgraph_ckp);
  }
