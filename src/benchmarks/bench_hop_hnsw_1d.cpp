...
int main(int argc, char **argv) {
  ...
  std::string search_param = fmt::format("efs_{}", args.efs, args.nrel);
  ...
  L2Space space(d);
  HopHnsw<float> comp(&space, nb, args.M, args.efc);
  ...
  if (fs::exists(ckp_dir / graph_ckp)) {
    comp.LoadGraph((ckp_dir / graph_ckp).string(), &space);
...
// #pragma omp parallel for num_threads(args.nthread) schedule(static)
  ...
  for (int j = 0; j < nq; j += args.batchsz) {
    comp.SearchKnn<float>(xq + j * d, args.batchsz, args.k, args.efs, &pred, metrics);
  }
  ...
  for (int j = 0; j < nq;) {
    vector<Metric> metrics(args.batchsz, Metric(nb));
    auto search_start = high_resolution_clock::now();
    auto results = comp.SearchKnn<float>(xq + j * d, args.batchsz, args.k, args.efs, &pred, metrics);
    auto search_stop = high_resolution_clock::now();
