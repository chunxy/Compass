...
#include "config.h"
#include "json.hpp"
#include "methods/Compass1dPcaQicg.h"
#include "utils/Pod.h"
...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassPcaQicg";
  ...
  Compass1dPcaQicg<float, float> comp(
      nb, d, args.dx, new L2SpaceB(args.dx), args.M, args.efc, args.nlist, args.M_cg, args.batch_k, args.delta_efs
  );
  ...
  std::string cgraph_ckp = fmt::format(COMPASS_X_QCGRAPH_CHECKPOINT_TMPL, args.nlist, args.M_cg, 200, args.dx, 8);
  std::string sq_ckp = fmt::format(COMPASS_SQ_CHECKPOINT_TMPL, args.dx, 8);
  ...
  if (fs::exists(ckp_root / "PCA" / c.name / cgraph_ckp) && fs::exists(ckp_root / "PCA" / c.name / sq_ckp)) {
    comp.LoadClusterGraph((ckp_root / "PCA" / c.name / cgraph_ckp).string());
    comp.LoadScalarQuantizer((ckp_root / "PCA" / c.name / sq_ckp).string());
    ...
    comp.SaveScalarQuantizer(ckp_root / "PCA" / c.name / sq_ckp);
  }
