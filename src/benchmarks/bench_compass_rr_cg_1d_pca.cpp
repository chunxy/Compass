...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassRRCg1dPca";
  ...
  CompassR1d<float, float> comp(d, args.M, args.efc, nb, args.nlist, args.dpca);
  ...
  if (fs::exists(ckp_root / "PCA" / c.name / pca_ivf_ckp)) {
    ...
    comp.SavePcaIvf(ckp_root / "PCA" / c.name / pca_ivf_ckp);
  }
  ...
  if (fs::exists(ckp_root / "PCA" / c.name / pca_rank_ckp)) {
    ...
    comp.AddPcaIvfPoints(nb, xb, labels.data(), attrs.data());
  ...
  const float *xqx = comp.ApplyPcaIvf(nq, xq);
