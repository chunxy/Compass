...
#include "config.h"
#include "json.hpp"
#include "methods/Compass1dKQicg.h"
#include "utils/Pod.h"
...
int main(int argc, char **argv) {
  ...
  std::string method = "CompassBikmeansQicg";
  ...
  Compass1dKQicg<float, float> comp(
      nb, d, new L2SpaceB(d), args.M, args.efc, args.nlist, args.M_cg, args.batch_k, args.initial_efs, args.delta_efs
  );
  ...
  std::string cgraph_ckp = fmt::format(COMPASS_QCGRAPH_CHECKPOINT_TMPL, args.nlist, args.M_cg, 200, 8);
  std::string sq_ckp = fmt::format(COMPASS_SQ_CHECKPOINT_TMPL, d, 8);
  ...
  if (fs::exists(ckp_root / "BisectingKMeans" / c.name / cgraph_ckp) &&
      fs::exists(ckp_root / "BisectingKMeans" / c.name / sq_ckp)) {
    comp.LoadClusterGraph((ckp_root / "BisectingKMeans" / c.name / cgraph_ckp).string());
    comp.LoadScalarQuantizer((ckp_root / "BisectingKMeans" / c.name / sq_ckp).string());
    ...
    comp.SaveScalarQuantizer(ckp_root / "BisectingKMeans" / c.name / sq_ckp);
  }
