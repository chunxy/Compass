...
class Rng : public AlgorithmInterface<dist_t> {
 public:
  ...
  Rng(SpaceInterface<dist_t> *s, const std::string &location, bool nmslib = false, size_t max_elements = 0) {
...
//                    if (candidate_id == 0) continue;
  ...
  linklistsizeint *get_linklist0(tableint internal_id, char *level0_memory_) const {
    return (linklistsizeint *)(level0_memory_ + internal_id * size_data_per_element_ + offsetLevel0_);
  }
      ...
      if (!is_cur_c_present) {
        if (sz_link_list_other < Mcurmax) {
          ...
          for (size_t j = 0; j < sz_link_list_other; j++) {
            candidates.emplace(
                fstdistfunc_(
                    getDataByInternalId(data[j]), getDataByInternalId(selectedNeighbors[idx]), dist_func_param_
                ),
                data[j]
            );
          }
