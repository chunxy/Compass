...
template <typename dist_t, typename attr_t> class CompassROld1d {
public:
  L2Space space_;
...
public:
  CompassROld1d(size_t d, size_t M, size_t efc, size_t max_elements,
                size_t nlist, size_t nrel, size_t nbits);
  ...
  vector<priority_queue<pair<float, hnswlib::labeltype>>>
  SearchKnn(const void *query, const int nq, const int k, const attr_t &l_bound,
            const attr_t &u_bound, const int efs, const int min_comp,
            const int nthread, vector<Metric> &metrics,
    ...
    this->ivf_->quantizer->search(nq, (float *)query, nprobe, distances,
                                  ranked_clusters);

    vector<priority_queue<pair<dist_t, labeltype>>> results(
        nq, priority_queue<pair<dist_t, labeltype>>());
        ...
        for (int level = hnsw_.maxlevel_; level > 0; level--) {
          ...
          while (changed) {
            ...
            for (int i = 0; i < size; i++) {
              ...
              dist_t d = hnsw_.fstdistfunc_((float *)query + q * ivf_->d,
                                            hnsw_.getDataByInternalId(cand),
                                            hnsw_.dist_func_param_);
      ...
      while (true) {
        ...
        if (candidate_set.empty() ||
            (!top_candidates.empty() &&
             -candidate_set.top().first > top_candidates.top().first)) {
          while (crel < nrel_) {
            if (itr_beg == itr_end) {
              if (++curr_ci == (q + 1) * nprobe)
                ...
                itr_beg =
                    btrees_[ranked_clusters[curr_ci]].lower_bound(l_bound);
                itr_end =
                    btrees_[ranked_clusters[curr_ci]].upper_bound(u_bound);
                continue;
              }
            }
            ...
            itr_beg++;
#ifdef USE_SSE
            _mm_prefetch(hnsw_.getDataByInternalId((*itr_beg).second),
                         _MM_HINT_T0);
#endif
            if (visited[tableid])
              continue;
            ...
            auto dist = hnsw_.fstdistfunc_((float *)query + q * ivf_->d, vect,
                                           hnsw_.dist_func_param_);
            metrics[q].ncomp++;
            crel++;

            auto upper_bound = top_candidates.empty()
        ...
        hnsw_.ReentrantSearchKnn((float *)query + q * ivf_->d, k, -1,
                                 top_candidates, candidate_set, visited, &pred,
  ...
  void LoadRanking(fs::path path, attr_t *attrs) {
    ...
    for (int i = 0; i < hnsw_.max_elements_; i++) {
      for (int j = 0; j < ivf_->nlist; j++) {
        in.read((char *)(ranked_clusters_ + i * ivf_->nlist + j),
                sizeof(faiss::idx_t));
      }
    }
    for (int i = 0; i < hnsw_.max_elements_; i++) {
      attrs_[i] = attrs[i];
      btrees_[ranked_clusters_[i * ivf_->nlist]].insert(
          std::make_pair(attrs[i], (labeltype)i));
    }
  }
  ...
  void SaveRanking(fs::path path) {
    ...
    for (int i = 0; i < hnsw_.max_elements_; i++) {
      for (int j = 0; j < ivf_->nlist; j++) {
        out.write((char *)(ranked_clusters_ + i * ivf_->nlist + j),
                  sizeof(faiss::idx_t));
      }
    }
  }
...
CompassROld1d<dist_t, attr_t>::CompassROld1d(size_t d, size_t M, size_t efc,
                                             size_t max_elements, size_t nlist,
                                             size_t nrel, size_t nbits)
    : space_(d), hnsw_(&space_, max_elements, M, efc), quantizer_(d),
      ivf_(new faiss::IndexIVFFlat(&quantizer_, d, nlist)),
      attrs_(max_elements, std::numeric_limits<attr_t>::max()),
      btrees_(nlist, btree::btree_map<attr_t, labeltype>()), nrel_(nrel) {
  ivf_->nprobe = nlist;
}

template <typename dist_t, typename attr_t>
int CompassROld1d<dist_t, attr_t>::AddPoint(const void *data_point,
                                            labeltype label, attr_t attr) {
...
template <typename dist_t, typename attr_t>
int CompassROld1d<dist_t, attr_t>::AddGraphPoint(const void *data_point,
                                                 labeltype label) {
  hnsw_.addPoint(data_point, label, -1);
  return 1;
}
...
template <typename dist_t, typename attr_t>
int CompassROld1d<dist_t, attr_t>::AddIvfPoints(size_t n, const void *data,
                                                labeltype *labels,
                                                attr_t *attr) {
