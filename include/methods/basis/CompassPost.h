 ...
 public:
  ...
  CompassPost(
      ...
      : n_(n),
        ...
        query_cluster_rank_(new faiss::idx_t[nlist]) {
  ...
  vector<priority_queue<pair<dist_t, labeltype>>> SearchKnnPostFilteredTwoHopWoCg(
      const void *query,
      const int nq,
      const int k,
      const attr_t *attrs,
  ...
  vector<priority_queue<pair<dist_t, labeltype>>> SearchKnnPostFilteredTwoHopRevisionNegation(
      const void *query,
      const int nq,
      const int k,
      const attr_t *attrs,
      const attr_t *batch_values,
      const int efs,
      const int nrel,
      const int nthread,
      BatchMetric &bm
    ...
    for (int q = 0; q < nq; q++) {
      BaseFilterFunctor *filter = new NegationQuery<attr_t>(batch_values[q], attrs, this->da_);
#ifndef BENCH
      auto q_start = std::chrono::high_resolution_clock::system_clock::now();
#endif
      vl->reset();
      ...
      graph_.SetSearchParam(k, k, k);
      // graph_.SetSearchParam(k, efs, k); // For testing non-iterative version.
      auto state = graph_.OpenTwoHop(query_q, graph_.hnsw_->max_elements_, filter, vl);
      // graph_.SetSearchParam(k / 2, k + k / 2, k / 2);
#ifndef BENCH
      auto graph_stop = std::chrono::high_resolution_clock::system_clock::now();
      auto graph_time = std::chrono::duration_cast<std::chrono::nanoseconds>(graph_stop - graph_start).count();
      ...
      while (top_candidates.size() < efs) {
        ...
        if (!ivf_finished && (nround_graph >= 1 && (state.sel_ <= breaktie || graph_last_round == 0))) {
          ...
          if (!initialized) {
            ...
            int clus = next.second;
            cclus = clus;
            second_range_used = false;
#ifndef BENCH
            auto btree_start = std::chrono::high_resolution_clock::system_clock::now();
#endif
            itr_beg = btrees_[clus].lower_bound(std::numeric_limits<attr_t>::min());
            itr_end = btrees_[clus].upper_bound(batch_values[q] - 1e-5);

            while (itr_beg != itr_end && !second_range_used) {
              if (itr_beg == itr_end) {
                if (second_range_used) {
                  break;
                } else {
                  itr_beg = btrees_[clus].lower_bound(batch_values[q] + 1e-5);
                  itr_end = btrees_[clus].upper_bound(std::numeric_limits<attr_t>::max());
                  second_range_used = true;
                }
              }
              bool good = (*filter)(itr_beg->second.first);
              if (good) {
                break;
              } else {
                itr_beg++;
          ...
          while (crel < nrel) {
            if (itr_beg == itr_end) {
              if (!second_range_used) {
                itr_beg = btrees_[cclus].lower_bound(batch_values[q] + 1e-5);
                itr_end = btrees_[cclus].upper_bound(std::numeric_limits<attr_t>::max());
                second_range_used = true;
                continue;
              }
#ifndef BENCH
        ...
        if (ivf_finished || nround_graph == 0 || state.sel_ >= breaktie) {
          ...
          auto graph_start = std::chrono::high_resolution_clock::system_clock::now();
#endif
          graph_.NextBatchTwoHop(&state, filter);
          if (ivf_finished && state.recycled_candidates_.empty()) {
            break;  // No more candidates to recycle.
          }
          priority_queue<pair<dist_t, labeltype>> &batch = state.result_set_;
